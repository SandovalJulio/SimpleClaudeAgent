// Área "configuración": modal con menú lateral; paneles General, Memorias, Herramientas y Tareas programadas.
module.exports = async ({ page, check, sleep, ACT }) => {
  const pane = async (name) => { await page.click(`.cfg-nav [data-pane="${name}"]`); await sleep(200); };
  await page.click("#open-config"); await sleep(500);
  check("abre configuración en General", await page.$eval("#overlay", (e) => e.classList.contains("open")) && (await page.textContent("#cfg-title")) === "General" && (await page.isVisible("#cfg-name")) && (await page.isHidden("#mem-edit")));
  check("seis secciones en el menú lateral", (await page.$$(".cfg-nav .nav-item")).length === 6);
  ...
  await pane("memorias");
  check("memorias cargadas en editor", (await page.isVisible("#mem-edit")) && (await page.inputValue("#mem-edit")).includes("## Memoria"));
