...
module.exports = async ({ page, check, sleep }) => {
  ...
  check("segunda conversación", (await page.$$("#convs .conv.open")).length === 2 && (await page.$$(".thread")).length === 2);
  ...
  check("cerrar conversación", (await page.$$("#convs .conv.open")).length === 1);
  ...
  check("búsqueda en la lista filtra", (await page.textContent("#convs")).includes("Sin coincidencias"));
